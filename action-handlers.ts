...
export const ActionHandlers = {
  goto: async (page: Page, step: StepConfig) => {
    if (!step.value) throw new Error("URL is required for goto");
    await page.goto(step.value);
  },
  
  expectText: async (page: Page, step: StepConfig) => {
    if (!step.value) throw new Error("Value is required for expectText");

    const element = page.getByText(step.value, { exact: false });
    await element.waitFor({ state: "visible", timeout: 5000 });
  },

  expectUrl: async (page: Page, step: StepConfig) => {
    if (!step.value) throw new Error("Value is required for expectUrl");
  ...
  fill: async (page: Page, step: StepConfig) => {
    if (!step.value)
      throw new Error(`Value is required for fill action on ${step.selector}`);
    const input = page.locator(
      `input[name="${step.selector}"], textarea[name="${step.selector}"]`,
    );
  ...
  autocomplete: async (page: Page, step: StepConfig) => {
    if (!step.value)
      throw new Error(`Value is required for autocomplete on ${step.selector}`);
  ...
  date: async (page: Page, step: StepConfig) => {
    if (!step.value)
      throw new Error(`Value is required for date on ${step.selector}`);

    const input = page
      .locator("label")
      .filter({ hasText: step.selector })
      .locator("..")
      .locator("input");
  ...
  select: async (page: Page, step: StepConfig) => {
    if (!step.value)
      throw new Error(`Value is required for select on ${step.selector}`);
